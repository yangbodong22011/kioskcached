...
{
    buffer_t *buffer = (buffer_t*)zmalloc(sizeof(buffer_t));
    if (buffer == NULL) {
        goto err;
    }
    buffer->buff = (unsigned char *)zmalloc(DEFAULT_BUFF_SIZE);
...
void check_buffer_size(buffer_t *buffer, size_t avlid_size)
{
    ...
    if (get_writeable_size(buffer) < avlid_size) {
        ...
        buffer->buff = (unsigned char *)zrealloc(buffer->buff, new_size);
